...
def create_inverse_pfile[F: np.floating, I: np.integer](
    ...
    *part: CLPartition[F, I] | None,
    ...
    dl_top, dl_interfaces = create_centerline_topology_list(mesh, tops, dl, field).unwrap()
    ...
    stiffness = create_stiffness_expressions(dm, top=dl_top).unwrap()
    # Loading and BCs
    motion_var = create_motion_variable(prob.motion_var, "CLDispt", tops, prob).unwrap()
    pres = {
    ...
    # Rigid Body
    rigid_probs = {
        s: create_rigid_body_constraints(tops, svar["i"].X, svar[s], mesh.GEO, motion=motion_t_prob)
        for s in ["0", "t"]
    }
    # Matrix
